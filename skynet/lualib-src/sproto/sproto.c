...
#define XTEA_MX (z >> 5 ^ y << 2) + (y >> 3 ^ z << 4) ^ (sum ^ y) + (k[p & 3 ^ e] ^ z)
#define XTEA_DELTA 0x9e3779b9

void _xtea_encrypt_(uint32_t *v, uint32_t len, uint32_t *k) {
	uint32_t n = len - 1;
...
void _xtea_decrypt_(uint32_t *v, uint32_t len, uint32_t *k) {
	uint32_t n = len - 1;
	uint32_t z = v[n], y = v[0], p, q = 6 + 52 / (n + 1), sum = q * XTEA_DELTA, e;
	if (n < 1) {
		return;
	}
	while (sum != 0) {
...
void xtea_encrypt(unsigned char *data, int size, unsigned char *key)
{
	_xtea_encrypt_((uint32_t*)data, size / 4, (uint32_t*)key);
}

void xtea_decrypt(unsigned char *data, int size, unsigned char *key)
{
	_xtea_decrypt_((uint32_t*)data, size / 4, (uint32_t*)key);
}
