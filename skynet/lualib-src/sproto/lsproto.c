...
// by lyx:xtea encrypt / decrypt
static int
lxteacrypt(lua_State *L,void (*crypt_func)(unsigned char *, int, unsigned char *)) {

	void * buffer = lua_touserdata(L, lua_upvalueindex(1));
	int sz = lua_tointeger(L, lua_upvalueindex(2));

	size_t dlen,klen;
	const char * data = luaL_tolstring(L,1,&dlen);
	const char * key = luaL_tolstring(L,2,&klen);

	char tkey[16];

	memset(tkey,0,16);
	memcpy(tkey,key,lyx_min(16,klen));

	if (sz < dlen)
		buffer = expand_buffer(L, sz, dlen);

	memcpy(buffer,data,dlen);

	crypt_func(buffer,dlen,key);

	lua_pushlstring(L, buffer, dlen);
	return 1;
}

// by lyx:xtea encrypt / decrypt
static int
lxteacrypt_c(lua_State *L,void (*crypt_func)(unsigned char *, int, unsigned char *)) {


	size_t dlen, klen;
	const char * key = luaL_tolstring(L,1,&klen);
	unsigned char * data = (unsigned char *)getbuffer(L,2,&dlen);
...
// by lyx:xtea encrypt 
static int
lxteaencrypt(lua_State *L) {

	return lxteacrypt(L,xtea_encrypt);
}

// by lyx:xtea encrypt 
static int
lxteadecrypt(lua_State *L) {
	return lxteacrypt(L,xtea_decrypt);
}

// by lyx:xtea encrypt 
static int
lxteaencrypt_c(lua_State *L) {

	return lxteacrypt_c(L,xtea_encrypt);
}

// by lyx:xtea encrypt 
static int
lxteadecrypt_c(lua_State *L) {
	return lxteacrypt_c(L,xtea_decrypt);
}
...
luaopen_sproto_core(lua_State *L) {
	...
	luaL_Reg l[] = {
		...
		// by lyx: xtea encrypt	（直接修改 c 数据）
		{ "xteaencrypt_c", lxteaencrypt_c },
		{ "xteadecrypt_c", lxteadecrypt_c },
	...
	// by lyx: xtea encrypt
	pushfunction_withbuffer(L, "xteaencrypt", lxteaencrypt);
	pushfunction_withbuffer(L, "xteadecrypt", lxteadecrypt);
