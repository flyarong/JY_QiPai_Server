...
static void
seri(lua_State *L, struct block *b, int len,const char * id) {
	uint16_t id_len=0;
	if(id)
	{
		id_len=(uint16_t)strlen(id);
	}
	uint8_t * buffer = skynet_malloc(len+2+id_len);
...
luaseri_unpack(lua_State *L) {
	...
	uint16_t head_len = *(uint16_t *)buffer;
	buffer = ((char *)buffer) + 2 + head_len;
	len-=(2+head_len);
	if (len == 0) {
		return 0;
	}
...
luaseri_jy_pack(lua_State *L) {
	...
	pack_from(L,&wb,1);
	assert(wb.head == &temp);
	seri(L, &temp, wb.len,id);
	wb_free(&wb);

	return 2;
}
int
luaseri_get_jy_id(lua_State *L) {
	if (lua_isnoneornil(L,1)) {
		return 0;
	}
	void * buffer;
	...
	lua_pushlstring(L,buffer,head_len);
