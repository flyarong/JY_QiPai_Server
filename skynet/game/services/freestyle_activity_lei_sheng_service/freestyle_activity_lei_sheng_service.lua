...
local function get_player_activity_award(_player_id,_broadcast)
	...
	if pad and pad.cur_process>=activity_config.max_process then
		...
		return_aw_tbl.data = ai

		ai = activity_config.award_id[ai]

		if _broadcast and activity_config.big_award[ai] then
			...
			if name then
				...
				skynet.timeout(1000,function ()
					skynet.send(DATA.service_config.broadcast_center_service,"lua",
							"fixed_broadcast","freestyle_activity_djhb_award",name
							,DATA.base_config.game_name,activity_config.broadcast[ai] or "大奖")
				end)
...
local function begin_activity()
	...
	-- skynet.send(DATA.service_config.broadcast_center_service,"lua",
	-- 				"fixed_broadcast","freestyle_activity_djhb_begin")
end
...
local function over_activity()
	...
	dump(log.activity_data , "--------------leisheng, log.activity_data:")
		...
		if not _score then
			...
			if player_activity_game_data[_player_id] == 1 then
				...
				if _score > 0 then
					...
					pad.cur_process = math.min(pad.cur_process,activity_config.max_process)
