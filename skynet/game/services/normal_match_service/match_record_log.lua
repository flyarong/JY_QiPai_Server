...
local match_log = {}
local match_player_log = {}


function CMD.add_race_id_log(_race_id)

    match_log.races[#match_log.races+1] = _race_id

end

function PUBLIC.add_match_log()
...
function PUBLIC.add_match_player_log(player_id,grades,rank,award)
...
function PUBLIC.start_match_log()
    ...
    match_log.game_id = DATA.match_game_id
    match_log.begin_time = os.time()
    match_log.player_count = #DATA.players
    match_log.races = {}
