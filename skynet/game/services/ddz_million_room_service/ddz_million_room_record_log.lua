...
local function get_id(_round,_race,_table_id)
    return _round.."_".._race.."_".._table_id
end
...
--[[对局日志
    _game_id,_issue,_round,_race,_begin_time,_end_time,_bomb_count,
    _spring,_base_score,_base_rate,_operation_list,_dizhu_seat,
    _players={
            player_id,
            score,
            rate,
...
local function add_race_log(log)
    ...
    skynet.send(DATA.service_config.data_service,"lua","add_ddz_million_race_log",
        1,
        log.issue,
        log.round,
        log.race,
...
local function concat_ddz_process_data_log(_id)
    local ddz_process_data=ddz_process_data_log[_id]
    if not ddz_process_data then
        print("error : save_ddz_process_data_log not found data ",_id)
        return ""
    end
...
function PUBLIC.save_ddz_process_get_permit_time(_game_table_data,_table_id)
	...
	local id = get_id(_d.game_config.round,
						_d.cur_race_num,
						_table_id)
...
function PUBLIC.save_race_start_log(_game_table_data,_table_id)
	...
	log[id].round=_d.game_config.round
	log[id].race=_d.cur_race_num
	log[id].issue = _d.issue
...
function PUBLIC.save_race_over_log(_game_table_data,_table_id)
	...
	for seat_no=1,3 do
		player_data[seat_no]={
			...
			score = _d.s_info.p_scores[seat_no],
