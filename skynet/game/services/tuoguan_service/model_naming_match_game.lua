...
function status.signuping.on_begin()
	...
	skynet.timeout(30,function ()
		local ret = PUBLIC.request_agent("nm_mg_signup",{id=DATA.game_info.game_id})
		if ret.result ~= 0 then
			print(string.format("tuoguan nm_mg_signup ,id=%s, error:%s",tostring(DATA.game_info.game_id),tostring(ret.result)))

			PUBLIC.send_to_agent("nm_mg_quit_game")
...
function status.signuped.update(dt)
	...
	if not status.signuped.sended_cancel and status_dur > 300 then
		...
		local ret = PUBLIC.request_agent("nm_mg_cancel_signup")
		if ret.result == 0 then
			PUBLIC.send_to_agent("nm_mg_quit_game")
			change_status(status.quit)
		end
...
function MSG.nm_mg_enter_room_msg(_data)
	PUBLIC.on_enter_room(_data.room_info, _data.players_info)
...
function MSG.nm_mg_join_msg(_data)
	PUBLIC.on_player_enter(_data.player_info)
end

function MSG.nm_mg_gameover_msg(_data)
...
-- 被踢出
function MSG.nm_mg_auto_cancel_signup_msg(_data)

	local ret = PUBLIC.request_agent("nm_mg_quit_game")
	if ret.result == 0 then
		change_status(status.quit)
	end
