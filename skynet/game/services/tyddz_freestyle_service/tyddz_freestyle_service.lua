...
require "normal_enum"
require "printfunc"
require "tyddz_freestyle_service.tyddz_freestyle_matching"
require "tyddz_freestyle_service.tyddz_freestyle_record_log"
...
--配置数据：配置表 match_data_config 中的数据 (by lyx 2018-4-23)
DATA.match_data_config = nil
--游戏过程数据 round => cfg
...
function CMD.player_signup(_player_id)
	...
	if "running" == DATA.current_service_status then
		...
		if player_info then
			...
			print("tyfreestyle player_signup ok:",_player_id)
...
local function stop_game()
	...
	--###test
	print("laizi freestyle stop_game")
	stop_game_state = 1
...
function CMD.reload_config(_cfg)
	...
	print(string.format("tyddz_freestyle_service config %d reloaded !",_cfg.id))
