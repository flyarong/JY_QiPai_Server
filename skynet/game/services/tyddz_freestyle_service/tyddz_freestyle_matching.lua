...
function PUBLIC.matching_players( _players )
	...
	while true do
		...
		local table_id=nodefunc.call(room_id,"new_table",
										...
										base_rate = DATA.game_config.base_rate,
										})
		...
		for i=curent_player_num-2,curent_player_num do
			...
			-- add_opt(player_id,function ()
				nodefunc.send(player_id,"tydfg_enter_room_msg",room_id,table_id)
				--###test
				print(string.format("tyfreestyle player:%s join table:%s",player_id,table_id))				
			-- end,2)
...
local function create_room_entries(room_num)
	...
	for i=1,room_num do
		...
		if room_id then
			...
			if not ret then
				skynet.fail(string.format("create_room_entries error:call tyddz_freestyle_room_service state:%s",state))
				return
			end
